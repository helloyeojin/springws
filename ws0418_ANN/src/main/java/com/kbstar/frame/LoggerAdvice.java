...
public class LoggerAdvice {
    ...
    @After("execution(* com.*.dao.ProductDao.get(..))")
    public void aftergetLog(JoinPoint jp) {
        Signature si = jp.getSignature();
        System.out.println("-----------After dao GET Log:" + si.getName());
    }

    @AfterReturning(pointcut = "execution(* com.*.service.ProductService.get(..))",
            returning = "returnVal")
    ...
    @AfterThrowing(pointcut = "execution(* com.*.service.ProductService.get(..))",
            throwing = "exObj")
